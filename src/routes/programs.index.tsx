  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="text-[11px] tracking-[0.25em] uppercase text-gold">{t("common.swipeHint")}</div>
            ...
            >
              <div className="aspect-[16/10] overflow-hidden relative">
                ...
                <div className="absolute inset-x-3 bottom-3 text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)]">
                  <div className="font-display text-xl md:text-2xl leading-tight line-clamp-1">{pick(p.name, lang)}</div>
                  <div className="text-[12px] text-white/95 line-clamp-1 mt-0.5">{pick(p.tagline, lang)}</div>
                </div>
              </div>
              <div className="p-4 flex-1 flex flex-col">
                <div className="flex items-center gap-1.5 text-[11px] text-navy/80">
                  <MapPin size={12} className="text-gold" /> <span className="line-clamp-1">{pick(p.venue, lang)}</span>
                </div>
                <ul className="mt-2.5 space-y-1 text-[12px] flex-1">
                  {p.highlights.slice(0, 3).map((h) => (
                    <li key={h.th} className="flex items-start gap-1.5 text-navy/85">
                      <span className="text-gold mt-0.5 shrink-0">✦</span> <span className="line-clamp-1">{pick(h, lang)}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-3 flex items-center justify-between pt-3 border-t border-mint/40">
                  <div>
                    <div className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground">{t("common.startFrom")}</div>
                  ...
                  <div className="rounded-full bg-emerald-deep text-ivory px-3 py-1.5 text-[11px] inline-flex items-center gap-1 group-hover:bg-gold group-hover:text-emerald-deep transition">
                    {t("common.viewDetail")} <ArrowRight size={12} />
                  </div>
                </div>
              </div>
            </Link>
