  ...
  head: () => ({
    meta: [
      { title: "Final Wellness Report — Goodfill Care" },
      {
        name: "description",
        content:
          "สรุปผล Before/After ครบทั้งการนอน, พลังงาน, HRV และความเครียด พร้อมแผน Long-term Care 90 วันสำหรับต่อยอดผลลัพธ์ของทริป Wellness ที่เกาะสมุย",
      },
      { property: "og:title", content: "Final Wellness Report — Before/After + แผน 90 วัน" },
      {
        property: "og:description",
        content:
          "ผลลัพธ์ Wellness แบบเป็นตัวเลข พร้อม Calm Credits และเส้นทางต่อเนื่อง 90 วันหลังออกจากเกาะสมุย",
      },
      { property: "og:url", content: "https://goodfillcare-samui.com/report" },
    ],
    links: [{ rel: "canonical", href: "https://goodfillcare-samui.com/report" }],
  }),
...
const metrics = [
  ...
  {
    icon: Battery,
    labelKey: "report.energy" as TKey,
    isStress: false,
    before: 5.1,
    after: 8.3,
    unit: "/10",
  ...
  {
    icon: Sparkles,
    labelKey: "report.stress" as TKey,
    isStress: true,
    before: 7.2,
    after: 3.1,
    unit: "/10",
  ...
  return (
    ...
    >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 md:gap-3">
            {metrics.map((m) => {
              ...
              return (
                <div
                  key={m.labelKey}
                  className="rounded-xl bg-pale-mint/30 border border-mint/40 p-3"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="text-[11px] md:text-xs text-navy/80 truncate">
                        {t(m.labelKey)}
                      </div>
                    </div>
                    <div
                      className={`text-[10px] flex items-center gap-0.5 shrink-0 ${positive ? "text-emerald" : "text-destructive"}`}
                    >
                      <TrendingUp size={10} /> {positive ? "+" : ""}
                      {((delta / m.before) * 100).toFixed(0)}%
                    </div>
                  </div>
                  <div className="mt-2 flex items-baseline gap-2">
                    <div>
                      <div className="text-[9px] uppercase tracking-widest text-navy/60">
                        {t("report.before")}
                      </div>
                      <div className="font-display text-base text-navy/70 line-through decoration-1">
                        {m.before}
                        {m.unit}
                      </div>
                    </div>
      ...
      <div className="mt-3 flex justify-end">
        ...
        <Link
          to="/care"
          className="btn-gold rounded-full px-5 py-2.5 text-xs inline-flex items-center gap-1.5"
        >
