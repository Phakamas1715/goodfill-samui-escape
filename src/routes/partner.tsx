...
function PartnerPage() {
  return (
    <DashShell bg="villa" host="welcome" kicker="Partner LIFF" title="คิวบริการวันนี้" subtitle="Bo Phut Beach Villa · LINE Partner">
      <div className="grid lg:grid-cols-[1fr,300px] gap-3 items-start">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <DashCard key={s.l} className="!p-3">
                  <div className="text-[9px] tracking-widest uppercase text-muted-foreground">{s.s}</div>
          ...
          <div className="flex gap-2">
            <Link to="/admin" className="btn-emerald rounded-full px-4 py-2 text-xs">→ Admin</Link>
            <Link to="/expert" className="card-cream rounded-full px-4 py-2 text-xs">→ Expert</Link>
          </div>
        </div>
        ...
        <DashCard className="!p-3">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {menu.map((m) => (
              <button key={m.en} className="bg-pale-mint/40 hover:bg-pale-mint aspect-square rounded-xl flex flex-col items-center justify-center gap-1 transition relative">
                <m.icon className="text-emerald" size={18} />
                <div className="text-[9px] font-medium text-navy">{m.th}</div>
                {m.count && <span className="absolute top-1 right-1 pill bg-gold text-emerald-deep px-1 py-0 text-[9px]">{m.count}</span>}
              </button>
            ))}
            </div>
        </DashCard>
